...
import { faPlus, faTrash, faArrowLeft, faSave, faSearch, faSpinner, faWrench, faTriangleExclamation } from '@fortawesome/free-solid-svg-icons'
...
const buildSelectStyles = () => ({
    control: (base, state) => ({
        ...
        borderWidth: '1.5px',
        boxShadow: state.isFocused ? '0 0 0 3px rgba(24,24,27,0.1)' : 'none',
...
export default function EditRequestPage({ params }) {
    ...
    const fetchShops = async () => {
        ...
        setShopsError(null)
        try {
            const res    = await fetch(ENV.API_SHOPS, { headers: getHeaders() })
            const result = await res.json().catch(() => ({}))
            if (!res.ok) throw new Error(result?.message || `خطای ${res.status} در دریافت شاپ‌ها`)
    ...
    const fetchTmCodesForShop = async (shop) => {
        setLoading(true)
        try {
            const res    = await fetch(ENV.API_TM_CODES, { headers: getHeaders() })
            const result = await res.json().catch(() => ({}))
            ...
            console.error('Error fetching TM codes:', err)
            setTmCodes([])
            Swal.fire('خطا', err.message || 'دریافت کدهای تعمیر ناموفق بود', 'error')
    ...
    const fetchMoreTmCodes = async () => {
        ...
        setLoadingMore(true)
        try {
            const res    = await fetch(`${ENV.API_TM_CODES}?cursor=${nextCursor}`, { headers: getHeaders() })
            const result = await res.json().catch(() => ({}))
            if (!res.ok) throw new Error(result?.message || `خطای ${res.status}`)
            const data   = result.data.tmCodes
        ...
        <div className="w-full min-h-screen" style={{ background: 'var(--bg)' }}>
            ...
            <div className="page-content max-w-7xl">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    {/* ⭐⭐ اضافه شد: اگه گرفتن شاپ‌ها خطا داد، واضح نشون بده */}
                    {shopsError && (
                        <div className="mb-4 p-3 rounded-xl flex items-center gap-2.5 text-sm alert alert-danger">
                            <FontAwesomeIcon icon={faTriangleExclamation} className="w-4 h-4 flex-shrink-0" />
                            <span><strong>خطا در دریافت لیست شاپ‌ها:</strong> {shopsError}</span>
                        </div>
                    )}
                                ...
                                <div className="grid grid-cols-2 gap-5">
                                    ...
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            options={shopOptions}
                                            styles={selectStyles}
                                            placeholder={shopsError ? 'خطا در دریافت شاپ‌ها' : 'جستجو نام یا کد شاپ...'}
                                            isDisabled={!!shopsError}
                                            isClearable
                                ...
                                <table className="w-full" style={{ borderCollapse: 'collapse' }}>
                                    ...
                                    <tr>
                                        {['شاپ', 'کد تعمیر', 'عنوان', 'تعداد', 'نوع', 'ساعت', 'هزینه', 'توضیحات', 'عملیات'].map((h, i, arr) => (
                                            <th key={h} className="px-3 py-3 text-center text-xs font-bold"
                                                style={{ color: 'var(--text-soft)', borderLeft: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
                                                {h}
                                            </th>
                                        ))}
                                    </tr>

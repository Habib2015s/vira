...
import { faPen, faSpinner, faPlus, faExchangeAlt } from '@fortawesome/free-solid-svg-icons'
...
export default function WarehouseTransfersPage() {
    ...
    const allData = data?.pages.flatMap(p => Array.isArray(p) ? p : (p?.data ?? [])) ?? []

    const columns = [
        { key: 'request_number', label: 'شماره', render: (row) => <span className="badge badge-primary">#{row.request_number}</span> },
        {

...
const BASE = ENV.API_MECHANISM_GROUPS

const fetchGroups = async ({ pageParam = null }) => {
    const url = pageParam ? `${BASE}?cursor=${pageParam}` : BASE
    const res = await fetch(url, { headers: getHeaders() })
    if (!res.ok) throw new Error('خطا')
    return (await res.json()).data.mechanismGroups
}

export default function MechanismGroupsPage() {
    const observerTarget = useRef(null)
    const queryClient    = useQueryClient()

    const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteQuery({
        queryKey: ['mechanismGroups'], queryFn: fetchGroups,
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mechanismGroups'] }); Swal.fire({ title: 'حذف شد!', icon: 'success', timer: 2000, showConfirmButton: false }) },
    ...
    const handleDelete = (row) => Swal.fire({
        title: 'حذف گروه', html: `گروه <strong>${row.name}</strong> حذف شود؟`,
    ...
    const columns = [
        ...
        { key: 'name', label: 'نام', render: (row) => <span className="font-bold" style={{ color: 'var(--text)' }}>{row.name}</span> },
        { key: 'created_at', label: 'تاریخ', render: (row) => <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{new Date(row.created_at).toLocaleDateString('fa-IR')}</span> },
        {
            ...
            render: (row) => (
                <div className="flex items-center justify-center gap-1.5">
                    <Link href={`/mechanism-groups/edit/${row.id}`}><button className="action-btn action-btn-edit"><FontAwesomeIcon icon={faPen} className="w-3.5 h-3.5" /></button></Link>
                ...
                <div className="page-header-bar">
                    <div className="max-w-7xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-black text-white leading-none">گروه‌های مکانیزم</h1>
                                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.75)' }}>({allData.length} گروه)</p>
                            </div>
                        </div>
                        <Link href="/mechanism-groups/create"><button className="btn btn-success"><FontAwesomeIcon icon={faPlus} className="w-4 h-4" />گروه جدید</button></Link>
                    </div>
                </div>
                <div className="page-content max-w-7xl">
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                        <DataTable data={allData} columns={columns} loading={isLoading && allData.length === 0} emptyMessage="هیچ گروهی یافت نشد" disablePagination={true} />
